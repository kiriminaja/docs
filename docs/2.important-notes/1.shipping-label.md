...
description: Shipping label for each package to be sent through the courier service using the KiriminAja platform with required and optional information
---
...
- :icon{name="material-symbols:check-circle"} Ensure all required fields are present and clearly visible on the shipping label.
- :icon{name="material-symbols:check-circle"} Verify that the AWB barcode is printed in 128A format.
- :icon{name="material-symbols:check-circle"} Confirm that the order reference barcode is printed in 128A format (if used).
- :icon{name="material-symbols:check-circle"} Double-check sender and recipient details for accuracy.
- :icon{name="material-symbols:check-circle"} Make sure the shipping label is securely attached to the package.
- :icon{name="material-symbols:check-circle"} Try to scan the AWB barcode and order reference barcode (if used) to ensure they are readable since the courier scanner device may vary.
...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JNE         | For JNE Shipping Label, please add information of `CASHLESS` before the `Shipping Type` if your delivery type is `Drop-Off`, no matter is COD or Non-COD. This is to inform the courier that the payment has been made online and no cash will be collected during delivery. That will look like: <wbr> CASHLESS COD: Rp500.000 or <wbr> CASHLESS Non-COD |
| J&T Cargo   | J&T Cargo require routing code to be printed on the shipping label. Please see our example here: [J&T Cargo Shipping Label](https://developer.kiriminaja.com/img/shipping-label-jtcargo.png)                                                                                                                                                              |
| SPX Express | SPX Require 2-column of sorting code to be printed on the shipping label. Please see our example here: [SPX Express Shipping Label](https://developer.kiriminaja.com/img/shipping-label-spx.png)                                                                                                                                                          |
...
![Shipping Label Sample](https://developer.kiriminaja.com/img/shipping-label-2.png)
