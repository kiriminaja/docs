...
| `payment_method`|             string  |    No    | Payment method for paid shipments `qris` for COD and QR response method or `credit` for automatically paid with KA Kredit          |
| `pin`           |             string  |    No    | PIN validation is only required for KA Kredit. For COD shipments, send `pin: ""`|
